...
if [ $? -eq 0 ]; then
    ...
    a developer install of jupyterlab_plotly you must have pip installed on
...
if [ $? -eq 0 ]; then
    ...
    if [[ "$OSTYPE" == "msys" ]]; then
        jupyter labextension install --py $nbExtFlags jupyterlab_plotly
    else
        jupyter labextension install --py --symlink $nbExtFlags jupyterlab_plotly
    fi
    jupyter labextension enable --py $nbExtFlags jupyterlab_plotly
else
    echo "'jupyter lab --version' failed, therefore jupyter lab is not installed. In
    order to perform a developer install of jupyterlab_plotly you must
...
if [ $? -eq 0 ]; then
    ...
    if [[ "$OSTYPE" == "msys" ]]; then
        jupyter nbextension install --py $nbExtFlags jupyterlab_plotly
    else
        jupyter nbextension install --py --symlink $nbExtFlags jupyterlab_plotly
    fi
    jupyter nbextension enable --py $nbExtFlags jupyterlab_plotly
else
    ...
    jupyterlab_plotly you must have jupyter notebook on your machine!
